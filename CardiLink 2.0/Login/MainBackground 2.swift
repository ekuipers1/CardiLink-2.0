...
//                            LinearGradient(gradient: gradient, startPoint: .top, endPoint: .bottom)) .frame(width: geometry.size.width * 0.95
...
struct TopViewLandscape: View {
    ...
    var body: some View {
        Curved_Shape(yOffset: wave1 ? -0.55 : 0.55)
        ...
        Curved_Shape(yOffset: wave2 ? -0.75 : 0.75)
            .fill(Color.colorCardiOrange)
            .frame(height: 100.0)
            .shadow(radius: 5, y: 5)
            .scaleEffect(x: -1, y: 1)
